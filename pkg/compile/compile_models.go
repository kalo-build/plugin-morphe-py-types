...
func generateModelContent(model *formatdef.Struct, config PythonConfig, morpheConfig cfg.MorpheConfig, r *registry.Registry) []byte {
	...
	if len(model.Fields) == 0 {
		...
		if config.UsePydantic && config.PydanticV2 && needsModelConfig {
			...
			cb.Dedent()
			cb.Line("}")
		} else if config.UsePydantic && !config.PydanticV2 && needsModelConfig {

...
import (
	...
	"github.com/kalo-build/plugin-morphe-py-types/internal/testutils"
...
func (suite *CompileTestSuite) TestMorpheToPython() {
	...
	config := compile.MorpheCompileConfig{
		...
		FormatConfig: compile.PythonConfig{
			UsePydantic:   true,
			PydanticV2:    true,
			AddTypeHints:  true,
			GenerateInit:  true,
	...
	compileErr := compile.MorpheToPython(config)
...
func (suite *CompileTestSuite) TestGroundTruthRegeneration() {
	...
	err := compile.MorpheToPython(config)
	suite.NoError(err)

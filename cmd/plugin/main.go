...
import (
	...
	"github.com/kalo-build/plugin-morphe-py-types/pkg/compile"
...
// PluginConfig represents the Python-specific configuration
type PluginConfig struct {
	// Python-specific settings
	PythonVersion string `json:"pythonVersion,omitempty"`
	UsePydantic   *bool  `json:"usePydantic,omitempty"`
...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: plugin-morphe-python-types <config>")
		fmt.Fprintln(os.Stderr, "  config: JSON string with inputPath, outputPath, and optional config parameters")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Example:")
		fmt.Fprintln(os.Stderr, `  plugin-morphe-python-types '{"inputPath":"./morphe","outputPath":"./output","verbose":true}'`)
	...
	logInfo(compileConfig.Verbose, "Output Python types to: '%s'", compileConfig.OutputPath)
	...
	// Pydantic settings
	if compileConfig.Config.UsePydantic != nil {
		morpheConfig.FormatConfig.UsePydantic = *compileConfig.Config.UsePydantic
		logInfo(compileConfig.Verbose, "Use Pydantic: %v", *compileConfig.Config.UsePydantic)
	}
	...
	if err := compile.MorpheToPython(morpheConfig); err != nil {

...
func generateEntityContent(entity *formatdef.Struct, morpheEntity yaml.Entity, config PythonConfig, r *registry.Registry) []byte {
	...
	if config.UsePydantic && config.PydanticV2 {
		...
		cb.Dedent()
		cb.Line("}")
	} else if config.UsePydantic && !config.PydanticV2 {
